...
def prototipo_telescopio(A, B_min, B_max, C, F, x1, y1):
    ...
    def update_plot(event):
        ...
        ax.quiver(x2_opt, y2_opt, vector_P[0], vector_P[1], angles='xy', scale_units='xy', scale=1, color='orange', label=r"$\vec{P}$")
        ax.quiver(x2_opt, y2_opt, vector_L[0], vector_L[1], angles='xy', scale_units='xy', scale=1, color='purple', label=r"$\vec{L}$")
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
...
# Ejemplo de uso
prototipo_telescopio(A=4, B_min=2.5, B_max=5, C=2, F=3, x1=-2, y1=6.5)
